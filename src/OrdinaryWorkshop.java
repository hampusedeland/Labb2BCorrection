...
//C
     ...
     */
    public class OrdinaryWorkshop<C extends Vehicle>{
        ...
        public String getInfoSpecificCar(int plats){
            return "The car at spot: "+ plats + " is a: " + loadedCars.get(plats).getModelName();
        }
        public void removeSpecificCar(int plats){
            Vehicle removed = loadedCars.remove(plats);
        ...
        public void addToloadedCars(C bil){
            if( !bil.isLoaded()) {
                if (loadedCars.size() < maxCars) {
                    ...
                    throw new IllegalArgumentException("Make sure the workshop isn't full");
                }
            }
            else {
